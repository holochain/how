...
export default (orchestrator) => {
  ...
  orchestrator.registerScenario('test profile zomes', async (s, t) => {
    ...
    const [alice_how_happ, bob_how_happ] = await installAgents(conductor,  ["alice", "bobbo"])
    const [alice] = alice_how_happ.cells
    const [bobbo] = bob_how_happ.cells

    // Trigger init to run in both zomes
    await alice.call('how', 'list_channels', { category: "General" });
    await bobbo.call('how', 'list_channels', { category: "General" });
